...
prior_resp <- make_sar_prior(basis_resp, tau = 0.01, rho = 1)

##################################################################
basis_cov <- make_piecewise_constant_basis(SpatialBasis::boston_watercover,values = c(1,0))
basis_prior <- make_flat_prior(basis_cov)

prior_cov <- make_sar_prior(basis_resp, tau = 0.01, rho = covariate_rho)
region_list <- build_region_list(target_grid)

grid_values <- integrate_region_list_parallel(region_list,integration_rule_area_overlap(),basis_cov)

W <- prior_cov$structure$W
I_minus_rho_W <- Matrix::diag(dim(W)[1])-covariate_rho*W

new_grid_values <- as.vector(solve(I_minus_rho_W,grid_values))
target_grid$diffuse_water <- as.vector(new_grid_values)

basis_cov <- make_piecewise_constant_basis(target_grid,values = new_grid_values)
basis_prior <- make_flat_prior(basis_cov)
...
prepared_list <- prepare_spatial_field_list(
  list(resid = response_field,cov = covariate_field,intercept_field = intercept_field),
...
(system.time(fit2 <- fit_stacked_spatial_field(
  stacked, orthogonalize = FALSE, return_variance = FALSE
)))
...
system.time(pred <- predict(fit, newdata = grid_coords, return_variance = FALSE, diagonal_only = FALSE))
